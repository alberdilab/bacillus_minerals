...
y_thresh <- 14          # example: -log10(p) > 5
lfc_thresh <- 2        # log2FC > 2 or < -2
...
```{r heatmap_top_genes_3h_DE, warning=FALSE, comments="", message=FALSE}


# Top 30 DE genes in Goethite vs Control
top_genes <- rownames(res_3h_G %>% arrange(adj.P.Val) %>% head(30))
...
top_genes <- rownames(res_3h_H %>% arrange(adj.P.Val) %>% head(30))
...
top_genes <- rownames(res_3h_M %>% arrange(adj.P.Val) %>% head(30))
...
```
```{r heatmap_top_genes_24h_DE, warning=FALSE, comments="", message=FALSE}


# Top 30 DE genes in Goethite vs Control
top_genes <- rownames(res_24h_G %>% arrange(adj.P.Val) %>% head(30))
...
# Top 30 DE genes in Hematite vs Control
top_genes <- rownames(res_24h_H %>% arrange(adj.P.Val) %>% head(30))
...
# Top 30 DE genes in Mica vs Control
top_genes <- rownames(res_24h_M %>% arrange(adj.P.Val) %>% head(30))
...
```{r test, warning=FALSE, comments="", message=FALSE, eval=FALSE}

library(rtracklayer)


gff_file <- "data/genomic_GB.gff"  # path to your downloaded file
gff <- rtracklayer::import(gff_file)

gene_gff <- gff[gff$type == "gene"]

gene_map <- as.data.frame(mcols(gene_gff)) %>%
  dplyr::select(ID, gene, locus_tag, product)
